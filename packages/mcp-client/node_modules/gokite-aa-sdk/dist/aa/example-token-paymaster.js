...
async function main() {
    ...
    if (!walletStatus.isDeployed) {
        ...
        const masterBudgetData = gokiteAccountInterface.encodeFunctionData("setMasterBudgetRules", [
            [BigInt(SECONDS_PER_DAY), 0n], // daily + per-tx
    ...
    const newSpendingRules = [
        {
            timeWindow: BigInt(SECONDS_PER_DAY), // daily limit
            budget: ethers_1.ethers.parseUnits("200", 18), // increased to 200 tokens/day
            initialWindowStartTime: currentDayStart(),
            targetProviders: [],
        },
        {
            timeWindow: 0n, // per transaction limit
            budget: ethers_1.ethers.parseUnits("20", 18), // increased to 20 tokens/tx
            initialWindowStartTime: 0n,
            targetProviders: [],
        },
        {
            timeWindow: BigInt(SECONDS_PER_DAY * 7), // weekly limit
            budget: ethers_1.ethers.parseUnits("500", 18), // 500 tokens/week
            initialWindowStartTime: currentDayStart(),
            targetProviders: [],
        },
    ];
