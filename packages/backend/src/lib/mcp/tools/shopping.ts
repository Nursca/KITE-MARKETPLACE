...
import { shopifyClient } from "@kite/x402-sdk";
...
const searchShopifyProductsTool = defineTool({
  ...
  handler: async ({ query }) => {
    const stores = storeRegistry.list().filter(s => s.isConnected && s.accessToken);
    if (stores.length === 0) {
    ...
    try {
      const products = await shopifyClient.searchProducts(store.shopUrl, store.accessToken!, query);
      return {
        success: true,
        products,
...
const buyShopifyProductTool = defineTool({
  ...
  inputSchema: z.object({
    ...
    shopUrl: z.string().describe("The Shopify store URL (e.g. store.myshopify.com)")
  }),
