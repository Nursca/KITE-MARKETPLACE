...
const erc8004_1 = require("@kite/x402-sdk/erc8004");
const blob_token_1 = require("@/lib/blob-token");
function parseBlobDescriptor(content) {
...
async function GET(req, res) {
    try {
        ...
        const buyerAddress = req.headers["x-buyer-address"] ||
            payerAddress ||
            "0xunknown";
        ...
        if (blob) {
            const appUrl = process.env.KITE_MARKETPLACE_URL ||
                process.env.NEXT_PUBLIC_APP_URL ||
                "https://kite-marketplace.vercel.app";
            const token = (0, blob_token_1.signDownloadToken)(blob.pathname);
