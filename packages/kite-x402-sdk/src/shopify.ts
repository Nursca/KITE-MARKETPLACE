...
export const shopifyClient = {
   ...
   */
  async request(shopUrl: string, accessToken: string, path: string, options: RequestInit = {}): Promise<any> {
   ...
   */
  async searchProducts(shopUrl: string, accessToken: string, query: string = ""): Promise<ShopifyProduct[]> {
    ...
    return data.products.map((p: ShopifyAdminProduct) => this.mapProduct(p));
  },

  /**
   * Get product details
   */
  async getProduct(shopUrl: string, accessToken: string, id: string): Promise<ShopifyProduct | null> {
   ...
   */
  async createOrder(shopUrl: string, accessToken: string, variantId: string, customerAddress: string, txHash: string): Promise<any> {
